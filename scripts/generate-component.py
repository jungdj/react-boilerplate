...
def story_template():
    ...
    lines.append('import { withNotes } from "@storybook/addon-notes"')
    lines.append('import %s from "./%s"' % (file_name, file_name))
    lines.append('')
    lines.append('import { BrowserRouter } from "react-router-dom"')
    lines.append('import { I18nextProvider } from "react-i18next"')
    lines.append('import { ThemeProvider } from "styled-components"')
    lines.append('')
    lines.append('import theme from "lib/theme"')
    lines.append('import i18n from "lib/i18n/index.stories"')
    lines.append('')
    lines.append('import props from "./%s.props"' % (file_name))
    lines.append('')
    lines.append('storiesOf("%s/%s", module).add(' % (outter_dir, file_name))
    lines.append('	"Default",')
    lines.append('	withNotes("Note")(() => (')
    lines.append('		<BrowserRouter>')
    lines.append('			<I18nextProvider i18n={i18n}>')
    lines.append('				<ThemeProvider theme={theme}>')
    lines.append('					<%s {...props} />' % (file_name))
    lines.append('				</ThemeProvider>')
    lines.append('			</I18nextProvider>')
    lines.append('		</BrowserRouter>')
    lines.append('	)),')
    lines.append(')')
    return '\n'.join(lines)
...
def test_template():
    lines = []
    lines.append('import React from "react"')
    lines.append('import ReactDOM from "react-dom"')
    lines.append('import %s from "./%s"' % (file_name, file_name))
    lines.append('')
    lines.append('import { BrowserRouter } from "react-router-dom"')
    lines.append('import { I18nextProvider } from "react-i18next"')
    ...
    lines.append('		div')
    lines.append('	)')
    lines.append('	ReactDOM.unmountComponentAtNode(div)')
    lines.append('})')
    return '\n'.join(lines)
...
if 'y' in targets or 't' in targets or '*' in targets:
    file_w = open(files_path['props'], 'w')
    file_w.write(props_template())
    file_w.close()
...
if 't' in targets or '*' in targets:
    file_w = open(files_path['test'], 'w')
    file_w.write(test_template())
    file_w.close()
